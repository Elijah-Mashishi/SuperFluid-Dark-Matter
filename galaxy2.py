...
#-----------------------Final expressions------------------------------#
    ...
    def v(self, Rs, Q):
        ...
        lamb = 0.05
        if hasattr(Rs, '__len__'):
            return [np.sqrt(1/R * (G * self.mass(R, Q) + np.sqrt((alpha**3 * lamb**2 / Mplanck) * G * self.mass(R, Q)))) for R in Rs]
        else:
            return np.sqrt(1/Rs * (G * self.mass(Rs, Q) + np.sqrt((alpha**3 * lamb**2 / Mplanck) * G * self.mass(Rs, Q))))
