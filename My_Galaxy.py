...
# ---------------- Rho_bulge ---------------------# 
    ...
    def rho_buldge_final(self, R):
        ...
        return -1 / np.pi * quad(func, 0, np.inf)[0]                           #Abel Transformation
...
#-----------------------Final expressions------------------------------#
    ...
    def mass(self, R, Q):
        return 4 / 3 * np.pi * R**3 * self.rho(R, Q)       #Total dark matter mass from equation (A.20) of 2201.07282v2

    def v(self, Rs, Q):
        α= 5.7
        Λ = 0.05
        if hasattr(Rs, '__len__'):
            return [np.sqrt(R * (G * self.mass(R, Q) + np.sqrt((α**3 * Λ**2 / Mplanck) * G * self.mass(R, Q)))) for R in Rs]
        else:
            return np.sqrt(Rs * (G * self.mass(Rs, Q) + np.sqrt((α**3 * Λ**2 / Mplanck) * G * self.mass(Rs, Q))))
